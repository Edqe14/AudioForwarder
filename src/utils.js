...
const processAudioStream = (s, format = 'adts', codec = 'aac', inf = 's32le') => {
  const f = ffmpeg({
    ...
    .audioBitrate(128e3)
    .audioChannels(2)
    .complexFilter('asetrate=48000*2^(1.09/12),atempo=0.046+(1/2^(1.045/12))')
    .audioCodec(codec)
    .format(format)
...
const processAudioStreams = (ss, format = 'adts', codec = 'aac') => {
  ...
  options = [
    ...options,
    `-filter_complex "amix=inputs=${ss.length}:duration=longest,asetrate=48000*2^(1.09/12),atempo=0.051+(1/2^(1.051/12))"`,
    '-b:a 128K',
    '-ac 2',
    `-c:a ${codec}`,
    `-f ${format}`
